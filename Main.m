...
% ===== Read Input Image =====
[f_name, d_name] = uigetfile('Check Images\*.*');
p_name = [d_name, f_name];
in_img = imread(p_name);
% in_img = imread('database\band cells\1.jpg'); % database\band cells\1.jpg
...
figure('name','PRe-processing Image','numbertitle','off');
imshow(uint8(img)),title('CMY');
...
% ===== Anfis =====
classes = Anfis_test(fea); % Anfic Classifier

if classes == 1
    msgbox('Affected image', 'message');
else
    msgbox('Not Affected image', 'message');
end