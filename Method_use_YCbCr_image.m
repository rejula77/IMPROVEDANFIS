function [seg_img, ba_img2] = Method_use_YCbCr_image(in_img)
...
y_img = rgb2ycbcr(in_img);
h_img = y_img;
ba_img2 = zeros(size(y_img,1), size(y_img,2));
ba_img2 = ba_img2 + 255;
for i = 1:size(y_img,1)
    for j = 1:size(y_img,2)
        if y_img(i,j) > 125 % Threshold 2
            ba_img2(i,j) = 0;
        end
    end
end
